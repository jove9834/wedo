...
use Wedo\Exception\FileNotFoundException;
...
class Filesystem {
	 ...
	 * @return array
	 */
	public function allFiles($directory, $include_path = FALSE)
	{
		...
		if ($fp = @opendir($directory)) {
			...
			while (FALSE !== ($file = readdir($fp))) {
				if (is_dir($directory.$file) && $file[0] !== '.') {
					$this->allFiles($directory.$file.DIRECTORY_SEPARATOR, $include_path);
				}
				elseif ($file[0] !== '.') {
					$_filedata[] = ($include_path === TRUE) ? $directory.$file : $file;
				}
			}
