<?php
/**
 * This file contains the error handler.
 * 
 * @author    黄文金
...
class CErrorHandler {
	...
	protected function handleException($exception) {
		...
		if($exception instanceof CHttpException || !WEDO_DEBUG) {
			$this->render('error', $data);
		} else {
			if($this->isAjaxRequest()) {
				Application::app()->displayException($exception);
			} else {
	...
	protected function handleError($event)
	{
		...
		if($this->isAjaxRequest())
			Application::app()->displayError($event->code,$event->message,$event->file,$event->line);
		elseif(WEDO_DEBUG)
			$this->render('exception',$data);
		else
			$this->render('error',$data);
	...
	{
		if(WEDO_DEBUG) {
			$version = '<a href="http://www.weidu178.com/" target="_blank">WEDO Framework</a>/' . Application::app()->getVersion();
	...
	protected function renderSourceCode($file,$errorLine,$maxLines)
	{
		...
		for($i=$beginLine;$i<=$endLine;++$i)
		{
			...
			$code=sprintf("<span class=\"ln".($isErrorLine?' error-ln':'')."\">%0{$lineNumberWidth}d</span> %s", $i+1, encode_string(str_replace("\t",'    ',$lines[$i])));
			if(!$isErrorLine)
				$output.=$code;
			else
